  ...
  return (
    <div className="py-[14px] flex items-center border-b-[.5px] border-b-slate-300 last:border-none justify-start md:px-4 lg:px-0">
      ...
      <div className="flex justify-between w-[84%] md:w-[87%] lg:w-[80%] items-center">
        <div className={`w-[55%] md:w-[79%] lg:w-[59%] pr-2 ${classTextBox}`}>
          <Link to={chatLink}>
            <p
              className={`text-sm md:text-xl lg:text-[15px] break-words font-semibold hover:text-primaryTwo cursor-pointer linear duration-300 ${classChtName}`}
            >
              {chatName}
            </p>
          </Link>
          <p
            className={`opacity-80 text-[11px] md:text-base lg:text-sm truncate ${classMsg}`}
          >
            {message}
          </p>
          <p
            className={`text-[10px] md:text-base lg:text-sm truncate ${classMsg}`}
          >
            {messageFooter}
          </p>
        </div>
        <div
          className={`w-[40%] md:w-[20%] lg:w-[40%] flex flex-col gap-y-1 justify-center items-center font-semibold text-[12px] md:text-lg lg:text-[14px] text-white ${classBtnBox}`}
        >
          <button
